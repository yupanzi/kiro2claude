...
| Codex 的工具怎么进来(两套形态)| `openai/responses/converter.ts`:`collectTools`(顶层 `tools` ∪ `input` 里的 `additional_tools`)+ `expandDefaultNamespace`(只就地展开 `functions` 容器,其余 namespace 原样留给 `convertTools` 丢弃);freeform 工具见 `FREEFORM_TOOL_SCHEMA`;踩坑「Codex code mode」|
...
| 客户端报「工具参数错误 / InputValidationError」怎么查 | 先看网关日志有没有 `upstream truncated tool_use (no isComplete frame)`(踩坑「截断 tool_use 谎报终态」);没有则多半在客户端侧——Claude Code 的 `InputValidationError` 有三支:`JSON parse failed`(残缺 input,即上面那条)、`ZOD_VALIDATION`(超 `questions≤4`/`options≤4`/`header≤12` 或违反「问题文本与同题内 option label 须唯一」的跨字段 refine,后两条 **JSON Schema 里表达不出**,模型看不到)、`PERMISSION_UPDATED_INPUT` |
...
- ★ **截断 tool_use 谎报终态(客户端侧表现为「首次调用某工具参数错误、重试就好」)**:上游偶发「宣告 tool_use、发了几段 input 分片、却从未发 `isComplete`」就断流。**两种形态必须分开**,判别子是 `hasContent()`:① **空壳帧**(有名字、零 input)→ `hasContent()` 假 → 走踩坑「空流有界重试」的确定性空流分支,单次定案不耗预算(`stream-handler.ts` 的 `truncatedToolUse` 问 `ctx.hasIncompleteToolUse()` 这个**事实**,不再靠终态恰好兜底成 `tool_use` 的巧合识别);② **已有 input 分片**→ 分片长度已计入 `outputTokens` → `hasContent()` 真 → 不判空 → 旧实现让 `closeOpenBlocks()` 无条件补 `content_block_stop`、终态兜底成 `tool_use`,客户端于是拿到**看似完整、实则 JSON 残缺**的工具调用,解析必然失败(Claude Code 报 `InputValidationError: JSON parse failed`)。形态②改判 `max_tokens`(**不覆盖**已显式定过的更具体终态),与 mid-stream Exception 同一条红线:**绝不静默截断成看似完整的 message_stop**。流式(`stream.ts` `generateFinalEvents` + `incompleteToolBlocks`)与非流式(`non-stream-reduce.ts` `incompleteToolIds`)**必须同源**——非流式那边残缺调用被直接丢弃,但 `hasToolUse` 在**宣告时**就置位,不修正会产出「终态 tool_use 却一个 block 都没有」。守卫 `test/claude/truncated-tool-use.test.ts`(含两条反向守卫:空壳帧仍报 `tool_use`、纯截断仍留在判空路径)。⚠ `hasContent()` 的**唯一定义点**是 `computeHasContent`,流式/非流式/两个 transport 都只填观测值——别再把实现内联回去。网关自己 abort/destroy 造成的截断由 `ctx.gatewayTruncatedUpstream` 打标、降 info,守卫 `test/static/gateway-cancel-contract.test.ts`(**位置式**检查:每个掐断点旁必须打标,新增掐断路径会直接红)
