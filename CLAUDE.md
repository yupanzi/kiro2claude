...
| 流里「有没有产出过真实内容」 | `StreamContext.hasContent()` = **唯一定义点**(三个分项与理由见其头注释);两个 transport 只留本地别名。判空、commit 触发、截断终态收窄三处共用 |
