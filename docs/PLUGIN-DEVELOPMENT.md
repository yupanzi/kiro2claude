...
1. **`enterprise/plugin-*/dist/index.js`** —— workspace-local 插件(仅在从开发 monorepo 运行时)
2. **`node_modules/**`** —— 任何 `package.json` 带 `kiro2claude-plugin` keyword 的包

若同一插件名在两处都出现,loader 在启动期报错。设 `KIRO2CLAUDE_PLUGINS_LOCAL=name1,name2` 可让列出的名字优先用 local 版本。
