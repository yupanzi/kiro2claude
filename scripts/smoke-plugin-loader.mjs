...
if (fs.existsSync(containerCore)) {
  ...
  // container: `pnpm deploy --legacy` flattens all core deps (incl. bundled
  // plugins) into /app/node_modules.
  ...
  // dev: pnpm does NOT hoist workspace plugins into the ROOT node_modules
  // (root has no dep on them); they live under core's per-package node_modules
  // because @kiro2claude/core depends on them. This mirrors core's own
  // resolvePluginRoot, which probes up to packages/core first.
  nodeModulesRoot = path.join(repoRoot, 'packages', 'core', 'node_modules');
} else {
...
// installed under repoRoot/node_modules (keyword 'kiro2claude-plugin').
