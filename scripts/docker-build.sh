...
echo "→ exec docker build -f docker/Dockerfile --build-arg KIRO2CLAUDE_CLI_VERSION=$VERSION ${PASSTHRU_ARGS[*]:-} ${EXTRA_TAG_ARGS[*]:-} ."
cd "$PROJECT_ROOT"
# core 镜像的 Dockerfile 在 docker/;build context 是仓根 `.`(Dockerfile 要
# COPY packages/ + fixtures/ + pnpm-workspace.yaml,收窄 context 会缺文件)。
exec docker build \
  -f docker/Dockerfile \
