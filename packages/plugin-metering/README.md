...
本 plugin 只做 credit **计量**,不做定价。USD 成本由独立的计价插件负责(闭源、单独分发;它单独读 `kiro.creditsUsed` host meta、输出 `usage.kiro_derived`),两者互不依赖。
...
USD 成本不在这里——需要金额时读 `usage.kiro_derived`(由独立的计价插件注入,本仓不含)。
