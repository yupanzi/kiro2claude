...
/**
 * Resolve the directory that contains the plugin sources — `<root>/enterprise/`
 * (workspace-local closed plugins) and `<root>/node_modules/` (npm/bundled free
 * plugins, e.g. @kiro2claude/plugin-metering).
 *
 * Must work across three layouts whose nesting depth differs, so a fixed
 * `'..','..'` from cwd cannot serve all three (cwd is also user-mountable, e.g.
 * the container's WORKDIR=/data):
 *   - dev:                <repo>/packages/core/{src,dist}/index → root=<repo>
 *   - free container:     /app/dist/index.js                    → root=/app
 *   - enterprise container:/app/dist/index.js + /app/enterprise → root=/app
 *
 * Strategy: derive candidate roots from THIS module's location (stable, not the
 * mountable cwd), then prefer the candidate that actually holds plugin sources.
...
function resolvePluginRoot(env: NodeJS.ProcessEnv): string {
  ...
  try {
    ...
    // dev needs up-3 (packages/core/{src,dist} → repo), container needs up-1
    // (/app/dist → /app). Probe each depth.
    const candidates = [
      path.resolve(dir, '..'),
      path.resolve(dir, '..', '..'),
      path.resolve(dir, '..', '..', '..'),
    ];
    // `enterprise/` uniquely marks the repo/app root in dev + enterprise image.
    for (const c of candidates) {
      if (fs.existsSync(path.join(c, 'enterprise'))) return c;
    }
    // Free image has no enterprise/ — fall back to the node_modules-bearing root.
    for (const c of candidates) {
...
async function main(): Promise<void> {
  ...
  //
  // Sources:
  //   - enterprise/plugin-*\/ dist (workspace-local closed-source plugins)
  //   - node_modules packages with keyword 'kiro2claude-plugin' (third-party)
  //
