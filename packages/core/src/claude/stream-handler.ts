...
export async function handleStreamRequest(
  ...
  // `!thinkingExtracted` is what keeps a reasoning-only stream out of the empty
  // bucket — it has outputTokens 0 but is not silent.
  const hasContent = (): boolean => ctx.outputTokens > 0 || ctx.thinkingExtracted;
  ...
  for (let attempt = 1; ; attempt++) {
    ...
    ctx = new StreamContext(model, inputTokens, extractThinking, toolNameMap, hookBus);
      ...
      safeWrite(
        ...
        sseEventToString(
          createSseEvent('error', {
            type: 'error',
            error: { type: 'overloaded_error', message: EMPTY_UPSTREAM_RESPONSE_MESSAGE },
          }),
        ),
      );
    }
