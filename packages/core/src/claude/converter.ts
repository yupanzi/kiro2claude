...
/** Appended to Write tool description */
...
/** Appended to Edit tool description */
...
/** Appended to system prompt: chunked write policy */
const SYSTEM_CHUNKED_POLICY =
 ...
 * 1. String: user_xxx_account__session_0b4445e1-f5be-49e1-87ce-62bbc28ad705
...
function convertTools(tools: Tool[] | undefined, toolNameMap: Map<string, string>): KiroTool[] {
  ...
  for (const t of tools) {
    ...
    // Limit description to 10000 characters (safe UTF-8 truncation).
    // UTF-16 .length is always >= the code-point count, so the cheap check
    // skips the common case without materializing a code-point array for
    // every tool on every request.
...
function mergeAssistantMessages(
  ...
  const content =
    contentParts.length === 0 && allToolUses.length > 0 ? ' ' : contentParts.join('\n\n');
...
export function convertRequest(
  ...
  const foldedMessages = foldSystemMessages(req.messages);
