...
describe('empty-stream retry: streaming path (app.inject)', () => {
  ...
  it('all empty (budget exhausted): real 503 overloaded_error, exactly maxAttempts calls', async () => {
    ...
    // 多次尝试全空 = 确定性空流:文案必须提示改内容(压缩会话),而不是
    // 误导客户端「please retry」——同一请求重试对确定性空流无效。
  ...
  it('retries=0 keeps single-attempt behavior (immediate 503, one call)', async () => {
    ...
    expect(res.statusCode).toBe(503);
    // 单次尝试无法区分瞬时与确定性 → 保留通用 retry 文案
    expect((res.json() as { error: { message: string } }).error.message).toContain('please retry');
  ...
  it('COMPLETE empty-input toolUse (isComplete, no args) surfaces as tool_use → 200, not retried', async () => {
    // Regression (2026-07, captured + replayed): a no-required-args
    // tool (browser_snapshot) is emitted by the upstream as ONE complete
    // toolUseEvent (stop=true) with empty input. It is a legitimate call — the
    // non-stream path returns it as a tool_use (200). The old stream `hasContent`
    // only counted outputTokens/thinking, so the same request 503'd on stream.
    // Must now: 200, emit a tool_use block, and NOT retry (single upstream call).
    const provider = queueProvider({
      stream: [
        () =>
          makeStreamResponse(
    ...
    expect(res.statusCode).toBe(200);
    expect(res.body).toContain('browser_snapshot');
    expect(res.body).toMatch(/"type":"tool_use"/);
    expect(res.body).toMatch(/event: message_stop/);
    // Legitimate content → no wasted retry.
    expect(provider.callApiStream).toHaveBeenCalledTimes(1);
  });
});
...
describe('empty-stream retry: non-streaming path', () => {
  ...
  it('COMPLETE empty-input toolUse surfaces as tool_use → 200 (stream/non-stream parity)', async () => {
    // Parity with the stream regression: the non-stream path already surfaces a
    // complete empty-input tool call (isComplete gate + empty buffer → input {}).
    // Pinned so the two paths can never diverge for this shape.
    const provider = queueProvider({
      buffer: [
        () =>
          makeBufferResponse([
