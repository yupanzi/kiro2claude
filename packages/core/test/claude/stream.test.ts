...
describe('SseStateManager.generateFinalEvents (hook bus shape)', () => {
  ...
  it('strips plugin extensions when request context flags stripPluginUsage (/api mirror)', () => {
    ...
    const hookEvent = makeHookEvent();
    hookEvent._setActivePlugin('test');
    hookEvent.addExtension('kiro_metering', {
      unit: 'credit',
      unitPlural: 'credits',
      usage: 0.005,
    });
    hookEvent.addExtension('kiro_derived', { costUsd: 0.01 });
    hookEvent.overrideStandardField('input_tokens', 4242, 'test override');

    const usage = requestContextStorage.run(
      { reqId: 'test', startTime: 0, stripPluginUsage: true },
      () => {
        const events = manager.generateFinalEvents(100, 50, hookEvent);
        return events.find((e) => e.event === 'message_delta')!.data.usage as any;
      },
    );
  ...
  it('keeps plugin extensions without the strip flag (/claude default unchanged)', () => {
    // 无标记（默认 /claude/v1）：扩展照常并入，确认镜像端点的过滤不误伤主端点。
    const manager = new SseStateManager();
    manager.handleMessageStart({ type: 'message_start' });
    ...
    // 显式在一个「无 stripPluginUsage」的请求上下文里跑，模拟 /claude/v1 作用域。
    const usage = requestContextStorage.run({ reqId: 'test', startTime: 0 }, () => {
      const events = manager.generateFinalEvents(100, 50, hookEvent);
      return events.find((e) => e.event === 'message_delta')!.data.usage as any;
    });

    expect(usage.kiro_metering).toEqual({ unit: 'credit', unitPlural: 'credits', usage: 0.005 });
  });
