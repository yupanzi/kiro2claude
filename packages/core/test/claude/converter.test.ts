...
describe('convertRequest - session ID extraction', () => {
  it('test_convert_request_with_session_metadata', () => {
    const req = baseRequest({
      ...
      metadata: {
        user_id:
          'user_0dede55c6dcc4a11a30bbb5e7f22e6fdf86cdeba3820019cc27612af4e1243cd_account__session_a0662283-7fd3-4399-a7eb-52b9a717ae88',
      } as Metadata,
    });

    const result = convertRequest(req);
    expect(result.conversationState.conversationId).toBe('a0662283-7fd3-4399-a7eb-52b9a717ae88');
  });
...
describe('convertRequest - identity override', () => {
  ...
  it('with-system branch: directive appended last, separated from chunked policy by a blank line', () => {
    // byte 级钉死生产常见的 if(systemContent) 分支的分隔符与顺序。'thinking prefix injection'
    // 的 withEmptySystem≡withNoSystem 只钉 else-if 分支(两操作数都无 system 内容);if 分支
    // 的 `${system}\n${SYSTEM_CHUNKED_POLICY}\n\n${directive}` 形态过去仅由顺序无关的
    // toContain 覆盖——把 \n\n 改回 \n、或把 directive 挪到 policy 前都不会变红。
    // SYSTEM_CHUNKED_POLICY 不导出,故用 endsWith/startsWith 钉相对形态而非整串 toBe。
    ...
    expect(head?.kind).toBe('user');
    if (head?.kind === 'user') {
      const content = head.userInputMessage.content;
      // directive 是末段且前导空行:回归成单 \n 或挪走 directive 即红。
      expect(content.endsWith(`\n\n${IDENTITY_OVERRIDE_DIRECTIVE}`)).toBe(true);
      // 顺序:客户端 system 原文在最前,紧跟单 \n 进入 chunked policy。
      expect(content.startsWith(`${systemText}\n`)).toBe(true);
    }
  });
});
...
describe('convertRequest - thinking prefix injection', () => {
  ...
  it('empty system (text: "") + thinking: treated as no-system, no empty system shell', () => {
    ...
    // 「无 system」,走 else 分支(thinking + identity),而不是拼出一个只含
    // SYSTEM_CHUNKED_POLICY 的空壳 system。用「与无 system 的结果逐字节相等」钉死它:
